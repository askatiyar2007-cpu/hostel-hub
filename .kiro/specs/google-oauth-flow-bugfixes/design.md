...
**Bug 2 (tab lost on Sign Up → "account exists" mismatch)**: This section was re-verified directly against the current `app/auth/callback/route.ts` and `app/auth/login/page.tsx` (both already read into context) rather than assumed from an earlier draft. The actual current behavior differs from what an earlier version of this design believed:

- The callback's two mismatch redirects (`reason=no-account`, `reason=signin`) already omit any `tab=` query parameter — they do **not** hard-code the opposite tab. This part of a previously-planned fix has, in effect, already happened.
- `app/auth/login/page.tsx`'s `reason`-handling `useEffect` already calls `router.replace(\`/auth/login?tab=${activeTab}\`)` (using the current `activeTab`, not a hard-coded value) and the action buttons already defer the tab switch to an explicit `onClick`. This part of a previously-planned fix has also already happened; **no change to `page.tsx` is required**.
- The remaining, real defect is narrower: because a Google OAuth round trip is a full browser navigation (not a client-side route change), all React state — including `activeTab` — is lost and re-initialized from scratch on the callback's redirect. The *only* channel by which the login page can know which tab the user started from is the `tab=` query parameter. Since the callback currently omits `tab=` on both mismatch redirects, the login page falls back to its default (`tab !== 'signup' → 'login'`). For `reason=no-account` (user started on Sign In) this default happens to coincide with the correct tab, masking the bug. For `reason=signin` (user started on Sign Up) it does not: the user is silently placed back on the Sign In tab, violating requirement 2.4.

The fix is a small, targeted change to the callback's two redirects: explicitly set `tab=` to the **starting** tab (`tab=login` for `no-account`, `tab=signup` for `signin`) instead of omitting it. This makes the login page's existing default-tab logic land on the correct tab in both cases, without any `page.tsx` change.
