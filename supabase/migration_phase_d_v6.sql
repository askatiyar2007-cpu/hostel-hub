/*
  PHASE D: DATABASE SCHEMA & SECURITY MIGRATION (V6 - TOTAL SCHEMA ALIGNMENT)
  Target Project: HostelHub (files/)
  
  V6 CHANGES:
  - Added comprehensive sync block for ALL hostel columns.
  - Added 'rules', 'amenities', 'description', 'contact_number', 'email', and 'cover_image_url' if missing.
  - Included 'NOTIFY pgrst, 'reload schema'' for immediate cache refresh.
*/
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='hostels') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='hostels' AND column_name='contact_number') THEN
      ALTER TABLE public.hostels ADD COLUMN contact_number TEXT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='hostels' AND column_name='email') THEN
      ALTER TABLE public.hostels ADD COLUMN email TEXT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='hostels' AND column_name='cover_image_url') THEN
      ALTER TABLE public.hostels ADD COLUMN cover_image_url TEXT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='hostels' AND column_name='rules') THEN
      ALTER TABLE public.hostels ADD COLUMN rules TEXT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='hostels' AND column_name='amenities') THEN
      ALTER TABLE public.hostels ADD COLUMN amenities TEXT[] DEFAULT '{}';
    END IF;
