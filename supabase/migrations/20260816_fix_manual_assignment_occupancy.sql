-- Fix manual assignment occupancy to use real-time active allocation count
-- This migration updates create_manual_assignment_with_invite to count actual active allocations
-- instead of relying on potentially stale rooms.occupancy/rooms.occupied_count columns
...
BEGIN
    ...
    -- f) Check if student already exists by email
    ...
    -- g) If student exists, check for active allocation
    ...
    -- h) Create the pre-approved room request to store parent, address, and emergency details
    ...
    -- i) Allocate the room using existing assign_student_to_room RPC with booking type
    ...
    -- j) Update room occupancy based on booking type (recalculate from source of truth)
    IF v_final_booking_type = 'entire_room'::public.booking_type THEN
        ...
        WHERE id = p_room_id;
    ELSE
        -- For shared bed, recalculate from actual active allocations
        UPDATE public.rooms 
        SET occupied_count = v_new_occupied,
            occupancy = v_new_occupied
        WHERE id = p_room_id;
    END IF;

    -- k) Generate 12 months of student fees using the existing create_student_fees RPC
    PERFORM public.create_student_fees(v_allocation_id);

    -- l) Create the invitation record
    ...
    -- m) Return JSON with status details
