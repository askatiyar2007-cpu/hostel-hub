...
-- Some deployments use the unified legacy OTP functions only, while others
-- also have the standalone password-reset functions. Lock down whichever
-- optional standalone functions exist without making this forward migration
-- depend on them.
DO $$
DECLARE
  ...
  FOR v_function IN
    ...
    WHERE n.nspname = 'public'
      ...
      AND p.oid::regprocedure::text IN (
        'request_password_reset_otp(text)',
        'verify_password_reset_otp(text,text)',
...
BEGIN
  ...
  LOOP
    EXECUTE format('ALTER FUNCTION %s SET search_path = public, auth', v_function);
  END LOOP;
END;
$$;
