-- Migration: Bulk Room Creation RPC Function
-- Date: 2026-09-02
-- Description: Create multiple rooms atomically with beds in a single transaction

-- Create or Replace the bulk room creation RPC function
...
DECLARE
  ...
  v_existing_room_numbers TEXT[] := '{}'::text[];
  v_index INTEGER := 0;
...
BEGIN
  ...
  LOOP
    ...
    IF v_room_number = ANY(v_existing_room_numbers) THEN
      RAISE EXCEPTION 'Duplicate room number % in batch', v_room_number;
    END IF;
    
    v_existing_room_numbers := array_append(v_existing_room_numbers, v_room_number);
  END LOOP;
...
-- Add comment
COMMENT ON FUNCTION bulk_create_rooms IS 'Atomically create multiple rooms with beds in a single transaction. Requires hostel ownership verification and prevents duplicate room numbers.';