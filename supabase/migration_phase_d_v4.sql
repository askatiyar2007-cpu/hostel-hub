/*
  PHASE D: DATABASE SCHEMA & SECURITY MIGRATION (V4 - FIX HOSTELS STATUS)
  Target Project: HostelHub (files/)
  
  V4 CHANGES:
  - Added missing 'status' column to hostels table.
  - Added missing 'area' and 'starting_price' columns to hostels table.
  - Fixed enum conversion from user_role to app_role using ::text cast.
  - Applied safe casting to both profile conversion and role migration.
*/
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='role' AND data_type IN ('text', 'USER-DEFINED')) THEN
    -- Attempt conversion if type is text or another enum (USER-DEFINED)
    -- Using role::text::public.app_role for safe transition between different enum types
    BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='hostels') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='hostels' AND column_name='status') THEN
      ...
      -- Mark existing hostels as approved
    ...
    -- Add missing starting_price column
    ...
    -- We check if owner_id references public.profiles(id)
    IF EXISTS (
      ...
      -- This is complex to do fully automatically in a DO block without naming the constraint,
      -- but we can at least migrate the DATA if it detects old IDs.
      -- If the first ID found in owner_id does not exist in auth.users, it likely needs migration.
      IF EXISTS (
  ...
  -- Using role::text::public.app_role to handle mismatches between profiles.role and user_roles.role types
  INSERT INTO public.user_roles (user_id, role)
  SELECT user_id, role::text::public.app_role FROM public.profiles
  ON CONFLICT (user_id, role) DO NOTHING;

  -- 4.6 Migrate Assignments (DYNAMIC COLUMN CHECK)
